  ...
  botonesComprar.forEach((producto) => {
    ...
    producto.addEventListener("click", function (e) {
      ...
      if (localStorage.carrito) {
        ...
        if (index != -1) {
          carrito[index].cantidad = carrito[index].cantidad + 1;   // si ya temgo un producto con ese id , le sumo uno a la cantidad

         
        } else {
          // sino lo agrego con push
          carrito.push({ id: e.target.dataset.id, talle: e.target.dataset.talle, cantidad: 1 });
        }

        // ACA CREAMOS EL CARRITO EN EL LOCALSTORAGE
        localStorage.setItem("carrito", JSON.stringify(carrito)); 
      } else {
        localStorage.setItem(
          "carrito",
          JSON.stringify([{ id: e.target.dataset.id, talle: e.target.dataset.talle, cantidad: 1 }])// creamos el carrito con el primer producto
        );
      }
